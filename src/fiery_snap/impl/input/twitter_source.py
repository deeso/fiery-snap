...
class TwitterClientImpl(object):
    ...
    def consume(self):
        ...
        try:
            ...
            logging.debug('Failed to retrieve posts with the following exception:\n{}'.format(traceback.format_exc()))
            try:
        ...
        for p in tweets:
            ...
            js['obtained_timestamp'] = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
...
class TwitterSource(IOBase):
    ...
    def handle(self, path, data):
        ...
        if path == JsonUploadPage.NAME:
            ...
            return {'msg': 'published %d messages in the queue' % len(messages)}
        ...
        if path == AddHandlesPage.NAME:
            ...
            all_posts = [i.toJSON() for i in posts]
            num_posts += len(k)
